...
export default class Damage {
    ...
    power(otherPokemonType) {
        ...
        const typeEffect = this.calculateTypeEffect(otherPokemonType)
        return {
            total: Math.floor(this.calculateStatsPower() * this.calculateStab() * typeEffect * critical * this.calculateOther() * this.getRandomFactor()),
            critical,
            typeEffect
        }
    }
    ...
    calculateStatsPower() {
        ...
        // TODO: remove this eventually (this is just for the demo)
        const level = 1; // this.attacking.stats.level

        // Calculate base power based on the pokemon's level
        const levelPowerbase = ((2 * level) / 5) + 2;
